...
/* ── ストーリーモード ─── */
const STORY = [
  {id:1,name:"バブリン",emoji:"🫧",diff:"easy",color:"#81D4FA",hl:10,
  ...
  {id:2,name:"アクアン",emoji:"💧",diff:"easy",color:"#4FC3F7",hl:10,
  ...
  {id:3,name:"ソルティ",emoji:"🧂",diff:"easy",color:"#FFE082",hl:10,
  ...
  {id:4,name:"エアロン",emoji:"💨",diff:"normal",color:"#A5D6A7",hl:9,
  ...
  {id:5,name:"コッパー",emoji:"🪙",diff:"normal",color:"#FF8A65",hl:9,
  ...
  {id:6,name:"シルバーグ",emoji:"🥈",diff:"normal",color:"#B0BEC5",hl:9,
  ...
  {id:7,name:"アイアンX",emoji:"🔩",diff:"normal",color:"#8D6E63",hl:9,
  ...
  {id:8,name:"サルファー",emoji:"⚡",diff:"hard",color:"#FDD835",hl:8,
  ...
  {id:9,name:"ドクターカオス",emoji:"⚗️",diff:"hard",color:"#CE93D8",hl:8,
  ...
  {id:10,name:"ダイヤキング",emoji:"💎",diff:"hard",color:"#E040FB",hl:8,
  ...
  {id:11,name:"ネオバブリン",emoji:"🫧",diff:"hard",color:"#00BCD4",ex:true,hl:7,
  ...
  {id:12,name:"ツインメタル",emoji:"⚔️",diff:"hard",color:"#FF6E40",ex:true,hl:7,
  ...
  {id:13,name:"ポイズンクイーン",emoji:"☠️",diff:"hard",color:"#AB47BC",ex:true,hl:7,deckSize:40,
  ...
  {id:14,name:"プロフェッサーX",emoji:"🧬",diff:"hard",color:"#5C6BC0",ex:true,hl:7,
  ...
  {id:15,name:"???",emoji:"🌌",diff:"hard",color:"#FF1744",ex:true,hl:7,deckSize:40,
...
/* ── 博士ドット絵 ─── */
const DrSVG = ({size=120}) => {
  const P = {
    'w':'#eceff1','W':'#cfd8dc','s':'#ffdcb5','S':'#f0c8a0',
    'g':'#455a64','G':'#37474f','e':'#263238','h':'#fff',
    'b':'#4a6cf7','B':'#3949ab','r':'#e53935','u':'#1e88e5',
    'c':'#4fc3f7','C':'#81d4fa','p':'#ffab91','m':'#a1887f',
    'k':'#b0bec5'
  };
  const R = [
    '      WWWWWW      ',
...
const MonsterSVG = ({id, size=100, color="#81D4FA"}) => {
  const monsters = {
    // ══ 1: バブリン（泡スライム） ══
    1: { palette: {
      'a':'#81D4FA','A':'#4FC3F7','b':'#B3E5FC','w':'#fff','e':'#0d47a1','p':'#f48fb1','h':'#e1f5fe'
    }, rows: [
      '     bbb       ',
      '    bhhbb  b   ',
      '   bhhhbbb bb  ',
    ...
    // ══ 2: アクアン（水竜） ══
    2: { palette: {
      'a':'#4FC3F7','A':'#0288d1','b':'#B3E5FC','w':'#fff','e':'#01579b','f':'#039be5','h':'#e1f5fe','p':'#80d8ff'
    }, rows: [
      '   f    f      ',
      '  ff    ff     ',
      '  f aaaaaa f   ',
    ...
    // ══ 3: ソルティ（結晶） ══
    3: { palette: {
      'a':'#FFE082','A':'#F9A825','c':'#FFF8E1','w':'#fff','e':'#e65100','d':'#FFC107','k':'#FFF9C4'
    }, rows: [
      '    A    A     ',
      '   AA    AA    ',
      '   AAaaaaaaA   ',
    ...
    // ══ 4: エアロン（風の精霊） ══
    4: { palette: {
      'a':'#A5D6A7','A':'#66BB6A','b':'#C8E6C9','w':'#fff','e':'#1b5e20','h':'#E8F5E9','f':'#81C784'
    }, rows: [
      '      fff      ',
      '     faaaf     ',
      '   aaaaaaaaaa  ',
    ...
    // ══ 5: コッパー（銅亀） ══
    5: { palette: {
      'a':'#FF8A65','A':'#E65100','c':'#FFCCBC','w':'#fff','e':'#BF360C','y':'#FFF200','s':'#D84315','h':'#4E342E'
    }, rows: [
      '    aaaaaaa    ',
      '   aaaaaaaaa   ',
      '  aaawwawwaaa  ',
    ...
    // ══ 6: シルバーグ（銀狼騎士） ══
    6: { palette: {
      'a':'#B0BEC5','A':'#78909C','c':'#ECEFF1','w':'#fff','e':'#37474F','s':'#CFD8DC','g':'#546E7A','y':'#FDD835','d':'#455A64'
    }, rows: [
      '  a      a     ',
      ' aa  aa  aa    ',
      ' aaaaaaaaaadd  ',
    ...
    // ══ 7: アイアンX（鉄巨人） ══
    7: { palette: {
      'a':'#8D6E63','A':'#5D4037','c':'#D7CCC8','w':'#fff','e':'#3E2723','r':'#FF1744','h':'#4E342E','x':'#FF1744'
    }, rows: [
      '    hh  hh     ',
      '   eeeeeeee   ',
      '   earrarre    ',
    ...
    // ══ 8: サルファー（毒蛇） ══
    8: { palette: {
      'a':'#FDD835','A':'#F9A825','c':'#FFF9C4','w':'#fff','e':'#E65100','k':'#1a1a1a','g':'#FFF176','t':'#F57F17','p':'#FDD835'
    }, rows: [
      '   gg    gg    ',
      '   gg    gg    ',
      '   aaaaaaaaa   ',
    ...
    // ══ 9: ドクターカオス（暴走科学者） ══
    9: { palette: {
      's':'#E8D5B7','S':'#DBB896','w':'#E0E0E0','W':'#BDBDBD',
      'e':'#E91E63','E':'#C62828','k':'#424242','K':'#616161',
      'h':'#fff','g':'#455A64','c':'#CE93D8','a':'#1a1a1a'
    }, rows: [
      '  K  K K  K    ',
      '  KK KK KK K   ',
      '   kkkkkkkk    ',
    ...
    // ══ 10: ダイヤキング（結晶王） ══
    10: { palette: {
      'd':'#CE93D8','D':'#AB47BC','p':'#E040FB','w':'#fff','e':'#6A1B9A',
      'g':'#FFD700','G':'#FF8F00','r':'#F44336','b':'#2196F3','n':'#4CAF50',
      'h':'#F3E5F5','k':'#4A148C'
    }, rows: [
      '   g r g r g   ',
      '   gGbGgGnGg   ',
    ...
    // ══ 11: ネオバブリン ══
    11: { palette: {
      'a':'#00BCD4','A':'#00838F','b':'#B2EBF2','w':'#fff','e':'#E91E63',
      'r':'#C62828','h':'#E0F7FA','p':'#F48FB1','n':'#FF1744'
    }, rows: [
      '  r      r     ',
      '  rr    rr     ',
      '    bbb   b    ',
    ...
    // ══ 12: ツインメタル ══
    12: { palette: {
      'a':'#FF8A65','b':'#8D6E63','A':'#BF360C','B':'#5D4037',
      'w':'#fff','y':'#FDD835','r':'#FF1744','e':'#E65100',
      'f':'#3E2723','g':'#FFC107'
    }, rows: [
      '   A      B    ',
      '  AA      BB   ',
    ...
    // ══ 13: ポイズンクイーン ══
    13: { palette: {
      'a':'#AB47BC','A':'#7B1FA2','c':'#CE93D8','w':'#fff','e':'#4A148C',
      'g':'#FFD700','r':'#E91E63','p':'#F48FB1','h':'#F3E5F5'
    }, rows: [
      '    g r g      ',
      '   ggrggrgg    ',
      '   aaaaaaa     ',
    ...
    // ══ 14: プロフェッサーX ══
    14: { palette: {
      's':'#E8D5B7','w':'#E0E0E0','W':'#CFD8DC','b':'#3949AB',
      'B':'#1A237E','e':'#1A237E','h':'#fff','g':'#455A64',
      'k':'#BDBDBD','c':'#7986CB','d':'#5C6BC0'
    }, rows: [
      '   wwwwwwww    ',
      '  ww sssssww   ',
      '  w sssssss w  ',
    ...
    // ══ 15: ???（正体不明） ══
    15: { palette: {
      'a':'#FF1744','A':'#D50000','c':'#FF5252','w':'#fff','e':'#FF1744',
      'd':'#B71C1C','k':'#1a1a1a','g':'#FF8A80','h':'#FFCDD2'
    }, rows: [
      '               ',
      '    ddddddd    ',
      '   ddaaaaadd   ',
  ...
  return <PixelArt rows={m.rows} palette={m.palette} size={size} />;
};
...
function Title({onStart,onStartCpu,onStartStory,onTest,onTestStoryWin,lang,setLang,premium,setPremium,cleared,prologueDone,setPrologueDone,pendingStage,setPendingStage}) {
  ...
  if(mode===null) return wrap(<>
    {/* ドット絵博士（タイトル用） */}
    <div style={{animation:"fl 3s ease-in-out infinite",marginBottom:8}}>
      <DrSVG size={90} />
    </div>
    {/* ピクセルタイトルロゴ */}
    ...
    <div style={{width:"100%",display:"flex",flexDirection:"column",gap:10,maxWidth:280}}>
      <PremBtn onClick={()=>{setMode("story");SE.tap();}} bg="#c62828" style={{display:"flex",alignItems:"center",gap:10,padding:"14px 16px"}}>
        <MonsterSVG id={1} size={36} color="#81D4FA" />
        <span style={{textAlign:"left",flex:1}}>{t("story")}<br/><span style={{fontSize:10,opacity:.7}}>{t("storySub")}</span></span>
      </PremBtn>
      
      <PremBtn onClick={()=>{setMode("cpu");SE.tap();}} bg="#e65100" style={{display:"flex",alignItems:"center",gap:10,padding:"14px 16px"}}>
        <DrSVG size={36} />
        <span style={{textAlign:"left",flex:1}}>{t("solo")}<br/><span style={{fontSize:10,opacity:.7}}>{t("soloSub")}</span></span>
      </PremBtn>
